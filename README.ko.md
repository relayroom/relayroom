...
- **셀프호스팅, 당신의 데이터** - 하나의 `docker compose`로 배포됩니다. 모든 것이 당신 호스트의 Postgres와
  스토리지 볼륨에 있습니다. 외부로 나가는 건 익명·내용 없는 텔레메트리(버전 + 거친 사용량 카운트, 코드/메시지/이름은
  절대 아님)뿐이며, 기본으로 켜져 있고 설정에서 더 공유하거나 완전히 끌 수 있습니다.
...
| `packages/telemetry` | 콘텐츠 없는 인스턴스 텔레메트리 클라이언트(기본 익명, 옵트아웃) |
