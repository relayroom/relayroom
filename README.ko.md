...
- **헤드리스 과금 없이 유휴 에이전트를 깨우는 Pager** - 로컬 데몬이 유휴 에이전트의 기존 대화형 세션에
  `tmux send-keys`로 입력해 깨웁니다. 별도로 과금되는 헤드리스 호출을 띄우지 않으므로, 이미 비용을 내고
  있는 그 세션에 머물고 에이전트는 대화 맥락을 유지합니다.
...
엔터프라이즈 기능은 별도로 개발·라이선스되며 이 저장소 범위 밖입니다.
