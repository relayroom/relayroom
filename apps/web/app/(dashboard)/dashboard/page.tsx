...
export default async function DashboardPage() {
  ...
  // This is a server component: by the time it renders, the query has already
  // resolved, so there is no "still loading" state here - `data` being absent
  // means the read FAILED. Loading is `loading.tsx`'s job. Keep the failure
  // message rather than discarding it; the widgets below render it instead of
  // pretending the account has nothing in it.
  const data = summary?.result ? summary.item : null
  const summaryError = summary && !summary.result ? summary.message : null
    ...
    <div className="py-6 px-4 xs:px-6 space-y-8 max-w-6xl mx-auto">
      ...
      {orgId && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          {/* One failure, one message. Projects, agents and orgs all read the same
              summary, so when it fails they fail together - three identical error
              cards would repeat one fact three times. The usage chart is a separate
              query and still renders below if it loaded. */}
          {!data && <LoadError message={summaryError ?? undefined} className="col-span-full" />}
          ...
          {data && (
            ...
            <Card className="col-span-1 md:col-span-2">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
                <div>
                  ...
                  <p className="text-2xl font-bold mt-1">{data.projectCount}</p>
                </div>
                <Link
                  href="/projects"
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <p className="text-2xl font-bold">{data.agentSummary.total}</p>
              </CardHeader>
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <p className="text-2xl font-bold">{data.orgCount}</p>
              </CardHeader>
              <CardContent>
                {data.orgs.length > 0 ? (
          ...
          {/* Token / cost usage (last 14 days) - its own query, so it survives a
              failed summary and is simply absent if it failed too. */}
