/**
 * RelayRoom monogram (two R's relaying). The main shape uses `currentColor` so it
 * adapts to the surrounding text color (navy in light, near-white in dark); the
 * relay stroke stays Teal — the one live brand accent.
 */
export function RelayRoomMark({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 166 126"
      className={className}
      role="img"
      aria-label="RelayRoom"
      fill="none"
    >
      <path
        fillRule="evenodd"
      ...
      <path
        fill="#14b8a6"
        d="M78.8 90.0L80.5 91.3L99.5 112.7L104.8 119.3L104.3 119.8L80.7 119.8L78.2 117.7L78.8 116.7L78.7 90.3Z"
      />
    </svg>
  )
}
