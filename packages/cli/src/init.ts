...
mcp_add() {
  ...
  if [ "$a" = "codex" ]; then
    ...
    RELAYROOM_TOKEN="$TOKEN" node -e 'const fs=require("fs"),os=require("os"),path=require("path");if(!process.env.RELAYROOM_TOKEN){console.error("agy MCP: RELAYROOM_TOKEN not set");process.exit(1)}const p=path.join(os.homedir(),".gemini","config","mcp_config.json");fs.mkdirSync(path.dirname(p),{recursive:true});let c={};try{c=JSON.parse(fs.readFileSync(p,"utf8")||"{}")}catch(e){}c.mcpServers=c.mcpServers||{};c.mcpServers.relayroom={serverUrl:process.argv[1],headers:{Authorization:"Bearer "+process.env.RELAYROOM_TOKEN}};fs.writeFileSync(p,JSON.stringify(c,null,2))' "$URL"
  else
