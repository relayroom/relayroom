...
# generated by relayroom ${CLI_VERSION}
set -euo pipefail

# WHICH CLI WROTE THIS FILE. rr.sh is generated into the worktree and never updates
# itself, so the installed CLI and this script drift apart silently - and the drift shows
# up as a flag that does nothing rather than as an error. \`up\` compares the two before it
# does anything else. Empty means a script older than this check, which is by definition
# stale: that is the only way the check reaches the scripts already written.
RR_GENERATED="${CLI_VERSION}"
...
# Is version $1 strictly newer than $2? Field-wise numeric, with parseInt so a
# prerelease suffix ("3-beta.1") compares as its number rather than as NaN.
_ver_newer() {
  node -e 'var a=String(process.argv[1]).split("."),b=String(process.argv[2]).split(".");
for(var i=0;i<3;i++){var x=parseInt(a[i],10)||0,y=parseInt(b[i],10)||0;if(x!==y)process.exit(x>y?0:1)}
process.exit(1)' "$1" "$2"
}

# REGENERATE THIS SCRIPT WHEN THE INSTALLED CLI HAS MOVED PAST IT.
#
# rr.sh is written into the worktree and does not update itself, so a machine can carry a
# current CLI and a script from three releases ago - and the symptom is not an error, it
# is a flag that silently does nothing (\`up --use-herdr\` on a 0.7.0 script started tmux
# and said so about nothing).
#
# There was already an update path, and it could not cover this: it fires on
# \`.relayroom/.update\`, which the PAGER writes from the hub's heartbeat reply. A worktree
# whose pager is dead never gets the marker - and a dead pager is exactly the state after
# a reboot, a herdr restart, or a month away, which is also when the script is most likely
# to be stale. That path answers "is there a newer CLI on npm"; this one answers "is the
# installed CLI newer than the script", which needs no hub and no pager.
#
# NO STAMP MEANS STALE. Every script written before this check has no RR_GENERATED, and
# treating that as "unknown, carry on" would leave exactly the scripts that need it. An
# installed CLI OLDER than the script is a warning and nothing more: regenerating would
# be a downgrade, and the person may have done it deliberately.
regenerate_if_stale() {
  [ -z "\${RR_SELF_UPDATED:-}" ] || return 0
  local installed
  installed="$($CLI --version 2>/dev/null | tr -d '[:space:]')" || true
  # No answer from the CLI is not evidence of anything - never regenerate on a guess.
  [ -n "$installed" ] || return 0
  if [ -n "\${RR_GENERATED:-}" ] && [ "$installed" = "$RR_GENERATED" ]; then return 0; fi
  if [ -n "\${RR_GENERATED:-}" ] && _ver_newer "$RR_GENERATED" "$installed"; then
    echo "rr: this rr.sh was written by relayroom $RR_GENERATED but the installed CLI is $installed." >&2
    echo "    Continuing on the script as written - regenerating would downgrade it." >&2
    return 0
  fi
  echo "rr: rr.sh was written by relayroom \${RR_GENERATED:-an older release}, CLI is $installed - regenerating"
  # Same regeneration the update path uses, and the same reason for --agent: without it
  # init falls back to "claude" and wires @RELAYROOM.md into a CLAUDE.md that a
  # codex/agy-only worktree has no use for. --force because the identity it is about to
  # write is the identity it just read from this worktree's own config.
  $CLI init --code "$CODE" --part "$PART" --server "$SERVER" --agent "$AGENT" --force --no-tmux-check >/dev/null 2>&1 || {
    echo "rr: could not regenerate rr.sh - continuing on the current script" >&2
    return 0
  }
...
# Only the verbs that then ACT on the script's own behaviour. \`status\` and \`statusline\`
# run constantly (the status bar polls), and a version check that re-execs underneath them
# would be a surprise with no benefit.
case "\${1:-help}" in
  up|launch) regenerate_if_stale "$@" ;;
esac
